...
**0. Choose. Select on the address column, not on tier.** User-named, else `cm-companies.csv` rows not yet in the tracker that **already carry a real address in `contact_1/2/3_email`** — the pattern is then free, and the pattern is the expensive half. Only when that pool is empty fall back to Tier A rows with a website.

**Do not filter to Tier A/B when building the queue.** Of the 134 address-bearing rows, 130 are labelled `C - unknown`. `target_tier` records co-man *confidence*, not address availability, and the two are uncorrelated — the D&B export supplied addresses without regard to whether the row had been qualified. A tier-first queue therefore selects *against* the rows that carry addresses and spends the run on address discovery instead, which converts at roughly 1 workable company in 5. This cost the 2026-07-30 run its whole day: 20 Tier A companies researched, 4 workable, 11 drafts, while 134 companies with addresses in hand sat untouched.

Volume comes from contacts per company, not companies per day. **50 drafts ≈ 12 companies at 4–5 ranked sends each**, not 50 companies at one email each. Screen the address pool per company before drafting — being a bulk export it contains brand owners that do not co-pack (Frito-Lay rows resolve to `pepsico.com`), foreign or off-thesis executives (Barilla's Switzerland MD), people mis-attributed to another employer, and mail domains that differ from the web domain (`thinntrim.com` for Old Neighborhood, `pimbrands.com` for Promotion In Motion, `csmingredients.com` for Brill). Promote every company you confirm from `C - unknown` to `A - verified` with its source URL, so the qualification is paid for once. Skip public companies. Skip rows with no website — there is no path to an address. Check the tracker for a direct competitor contacted the same week — if so, never mention one to the other, and avoid insights whose natural reply is "who else are you talking to?"
...
**Salutation.** Default is `Dear Mr. <Last>,` / `Dear Ms. <Last>,` — a courtesy title reads as a person who did the work, a bare full name reads as a mailmerge. The title must come from a **published source that states it**: the person's own bio using he/him or she/her, a press release calling them "Mr. X", an interview, an association profile. **Never infer a courtesy title from a first name** — a wrong one is worse than none, and this pipeline does not guess gender. When no source states it, fall back to `Dear <First>,`. A shared family surname is fine in a one-to-one email — two Dabrows each get `Dear Mr. Dabrow,` — and only drops to the first name when a single message addresses more than one of them. `Dear <First> <Last>,` is rejected by the script. Record the basis in the tracker Notes, e.g. `salutation: Ms. per company bio "she joined in 2019"`. **Recency bar — kill-signal test (revised 2026-07-31 by Aman's decision).** Draft to anyone who has a published address and a title **unless one search turns up evidence they have left.** Run `"<First> <Last>" <company>` once and read the top 5–10 results. Bench only on a kill signal: an aggregator profile showing a NEW employer, "former"/"ex-", retirement coverage, an obituary, or a role signal that predates the company's last ownership change. No kill signal found means draft them. An undated own-site listing is a positive signal, not a disqualifier — a live leadership page is the company asserting the role now.

*Why it was changed.* The rule this replaces required positive dated proof a person still held their role, and benched everyone who lacked it. At private family co-mans that proof is usually not published at all, so the bar was rejecting people for the absence of evidence rather than for evidence of absence. It cost about 4x the output: Rotella's had six contacts and yielded two, and a 20-company sweep yielded eleven drafts. Batch 1 delivered 13 of 15 under the looser standard, and its one person-failure (`pbegg@lyonsmagnus.com`) carried an explicit kill signal in the row — "no post-2024 confirmation he still holds the role" — which the one-search test catches. Expect bounces to rise toward 20–25%; the parallel general-inbox test on every batch is what surfaces them, and a bounce is recoverable while an unreachable contact is not.

Still benched regardless: off-thesis roles (HR, marketing), and anyone whose *address* cannot be derived because the domain shows two competing pattern shapes with no verbatim address for that person.
